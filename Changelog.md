...
## 2026-09-12（CL-0106） F-15 に計器: 閾値がバッチ相対である事実を出力に出す（挙動は不変）

### 概要
* percentile_gate が提出バッチの分位であることは設計どおり（7件→0.474 / 4件→0.554 の3実測）。**変えたのは、その依存を呼び手が読めなかった点だけ。**
* post-gate 診断に `percentile_gate 0.611（提出 6 件の上位30%点＝このバッチ次第で動く） / 固定フロア 0.2 なら通過 6 件` を追加し、固定フロア通過数が percentile 通過数を上回るときだけ「本日の落選はバッチで閾値が上がったことによるものを含む」と注記。
* 新規回帰2件（診断が両方の閾値と通過数を持つ／同一スコアが弱いバッチで通過し強いバッチで落ちる）。**458 → 460 tests: 460 pass。**

---

## 2026-09-12（CL-0105） F-26 対処: 生存シードの下限と一度だけの広い再取得——「多様な無関係」を通常の顔で返さない

### 概要
* 2026-09-09 の失敗（語彙 60 件中 49 が言語ゲート・9 が空参照で落ち、**2 件**の名簿から 30 件の交差候補を通常出力として返した）に対し、F-26 の処方 (i)(iii) を実装。
* **(iii)** 生存シードが下限未満かつ語彙シードが満杯なら **`seed_count × 9` で1回だけ取り直す**（ゲートは正しく供給が足りない型を救う）。**(i)** それでも下限（既定 5）未満なら**拒否**し、取得数・取得ラウンド・言語ゲート除外数・空参照除外数・semantic レッグ内訳・打ち手4つを返す。`min_seeds:0` で無効化。
* **下限 5 の根拠**: 失敗時 2 件 / 同時期の健全 run 20 件。**返さない方が正しい理由は「収穫ゼロ」ではなく「健全な run と見分けがつかない出力を返すこと」**（S-68 の出力版）。
* 新規回帰3件＋既存 bybridge 経路テストを `min_seeds:0` 明示へ。**455 → 458 tests: 458 pass。**
* **seihai 側への申し送り**: bybridge が結果を返さないことがある（故障ではない）。内訳を insights に記録してほしい。

---

## 2026-09-12（CL-0104） F-14/F-20 対処: byrepo の関連度を「語の出現数」から「キーワード網羅率」へ——8週続いた順位の無情報化を解く

### 概要
* **同じ欠陥が3つのコピーに入っていた**（`git_collect`/`hf_collect`/`kaggle_collect` が各自 `_theme_fit_score` を持っていた）。共有モジュール `src/pipeline/theme_fit.py` に統合。
* **直した3点**: (1) **部分一致**（`cuped`→`cups-filters`）を**正規化後の語境界一致**に、(2) **区切り文字の無視**（`quality-diversity` が散文 "quality diversity" に当たらない）を両側正規化で解消、(3) **飽和**（`credit×10` 上限 30 ＝ 3本当たれば上限）を**網羅率**（credit ÷ キーワード数）へ。
* **実測（実 GitHub API・9/10 の実テーマ）**: 関連度 1.00/1.00/1.00 の3件が **0.60/0.80/0.73** に分離し、順位が `openevolve > pyribs > QDax` → **`pyribs > openevolve > QDax`**。**seihai が 9/10 に報告した「主題直撃の pyribs が無関係なスキル集より下位」は実データで解消**。無関係な Agent-Skills は 0.33 → 0.20。
* **F-14（定型文）側**: 一致したキーワードと当たった面を `source_meta` に持たせ、structured / LLM 両経路の出力に `一致キーワード 4/5: quality-diversity（名前/説明/topics）/ archive（README・部分 0.4）` を印字。一致0件は「なし（0/5）」と明示。**散文の定型性自体は未対処**だが、その隣に repo 固有の決定論的証拠が並ぶ。
* **効かなかったこと**: 9/11 のテーマでは**順位が1つも動かなかった**（`freqtrade` 関連度 0.20 × Reliability 89 が `MSML` 関連度 0.40 × Reliability 54 に勝ったまま）。係数の床 0.35 が原因で、**逆転には床 < 0.098 が必要**と計算で確定。**F-29 として分離起票**（床を下げる／足切りにする／Reliability の可動域を圧縮する、のいずれも全順位を動かすので実テーマ複数本の before/after が要る）。
* 新規回帰6件（略語の部分一致 0／ハイフン語が散文に一致／飽和しない／**観測された逆転が反転する**／証拠行の描画／一致0件の明示）。旧スケールを固定していた既存8件を新契約へ更新。**449 → 455 tests: 455 pass。**
* **seihai 側への申し送り**: **関連度の意味が変わった**——「何本当たったか」ではなく「宣言したキーワードの何割を満たすか」。**キーワードを増やすほど1本の重みが下がる**（5本中2本＝0.4）。出力の `一致キーワード` 行を読めば、数値の根拠がその場で確認できる。

---

## 2026-09-12（CL-0103） F-16 対処（クエリ長の自動切り詰め＋400 の短縮再取得＋診断文の原因順）／F-28 は 429 の名指しのみ

### 概要
* **F-16**: `cap_semantic_query` で `structure + pseudo_abstract` を **1,200 字で文末切り詰め**（実測: 英語 1,575 字で 400・1,437 字は成功）。それでも 400 なら **600 字で1回だけ再取得**し、facet の `status` に短縮した事実を残す。**実測**: 9/04 の長さ条件（2,175 字）は旧経路で 400 → 新経路は切り詰め 1,118 字で **返却 50 → 候補 42 件**（*The Importance of Knowing When to Stop* 等、主題側）。
* **診断文も直した（9/04 にいちばん害があった部分）**: 0 件メッセージが「近すぎる／飽和」しか名指しせず、呼び手が生きたテーマを飽和と結論しかけた。新文面は status を読む順序を指示し、`400 →クエリ長`／`429 →共有クォータ`／`504 →一過性・単独再投`／`home_converged` で初めて「近すぎる・飽和」を疑う、と書く。
* **F-28**: リトライを使い切った 429 を「**この IP の共有クォータ枯渇。クエリや候補の問題ではない。時間をおいて再実行**」と名指しする（従来は汎用の `request failed after 3 attempts`）。**資源競合そのもの（by\* 横断のトークンバケット）は未実装**——複数セッションをまたぐ資源なので contra 内の状態では数えられない。呼び手側の規約（bybridge を先に回す）が当面の処方で、これは seihai の判断事項。
* 新規回帰5件（長さキャップと文末切り／短いクエリは不変／400 の短縮再取得が1回だけ走る／429 の文面契約／0 件メッセージが 400・429・504 を飽和より前に名指しする）。既存の「上限回数」テストは 429 から 503 へ移した（429 の文面は別契約になったため）。**444 → 449 tests: 449 pass。**
...
### 概要
* **F-21（3例＋本日発見の2例）**: スキーマの説明文が `system-building` を提示して検証器が拒否／`assumptions` が optional 表示なのに 2〜5 件必須／`theme_overview` が「3-6 sentences」表示で実際は 200〜1200 字／`scope_scale` は許容値がどこにも書かれていない。**呼び手は説明文どおりに書いて落ちる**ので、1回ぶんの往復が毎回失われていた。
* **実装**: `src/mcp_server.py` がスキーマを**検証器の定数から生成**（`enum` / `minItems` / `maxItems` / `minLength` / `maxLength`）＝MCP クライアント側で事前に弾ける。`assumptions` を4ツールの `required` に追加。拒否メッセージは許容集合と拒否値を載せる（`approach_type is invalid: 'system-building' (allowed: application, experiment, theory)`）。`bynote` の任意 `theme_overview` は巻き込まない。
* **乖離防止テスト**: 各ツールの `enum` が検証器の集合と厳密一致／**スキーマが提示する全値の直積が実際に検証を通る**（「提示したのに拒否」を構造的に不可能にする）／`required` と境界の一致／メッセージ内容。**433 → 437 tests**。
* **F-22 は計器のみ（スコア定義は不変）**: 落選行に `purpose_sim × mechanism_dist` と**律速側**を併記し、`purpose_sim ≥ 0.70` かつ距離が律速の落選を **★『近いが有用』**として名指し、「距離 × 構造である以上どの閾値でも通らない／採否の最終判断は呼び手」と出力に明記。
* **閾値 0.70 は実データで較正**: 9/12 の6件バッチ＋9/05 の CUPED を再生し、**seihai が「今週いちばん実務的」と書いた3本＋CUPED にだけラベルが付き、遠いが浅い2本（0.66/0.64）には付かない**ことを実測。**437 → 440 tests: 440 pass。**
* **正直に残す**: F-22 の本体（近い当たりが構造的に通過できない）は**未対処**。処方 (a)（`utility` 系の第2順序の併記）は残件。閾値調整もスコア定義変更もしていない。
* **seihai 側への申し送り**: 呼び出し側の書き換えは不要（今まで通っていた呼び出しはそのまま通る）。『近いが有用』は「捨てろ」ではなく「採否は呼び手が決める」の合図。
...
### 概要
* **直したのは並べ替えではなく取得。** seihai 9/07（F-23・最頻 bridge が `R` の引用で候補の 62%）と 9/08（F-24・`Theory of the firm` で 85%）は、**種別の違う bridge が同じ様式を作っていた**。共通の機序は種別でも被引用数でもなく、`cites:B1|B2|...|B50` という **1本の OR クエリからの抽出が各 bridge の被引用数に比例する**こと。50本プールのうち通行があったのが 3〜5 本だったのはその帰結。2026-08-22 の上位窓多様化はこの段の**後**の並べ替えなので、プールの成り立ちには触れていなかった。
* **実装（加算的・可逆・シグネチャ非変更）**: (1) `collect_citation_candidates` が bridge を**1本ずつ**問い合わせ、1本あたり `max_count // 10` 件（既定 6 件）を上限に供給する（`CollectConfig.bridge_fair_share`・既定 ON、`per_bridge_cap` で明示指定可）。走査順は**プール順**＝複数シードが共有する bridge 優先。(2) 不足分は従来の一括 OR 走査で補充＝recall の床。1本が HTTP エラーでもその bridge を飛ばすだけ。(3) `mcp_server` が渡していた `sorted(bridges)`（**id のアルファベット順**＝多様性順序の破棄）を順序付きプールに修正。(4) 配分の存在と上限を診断行に明記（数値だけでは「OR 1回の 25%」と「6件枠での 25%」を区別できないため）。
* **実測 before/after（同一シード名簿・同一プールで `bridge_fair_share` のみ切替）**: 新フィクスチャ `theme_seihai_retrigger_hysteresis.json`（F-24 のテーマ族・新規同梱）＝最頻 bridge 占有 35%→**25%**／通行 bridge 17→**27 本**／平均共有 bridge 1.55→**2.08**／単一 bridge 候補 44/60→**32/60**。既存 `theme_seihai_strategy_generation.json`＝27%→25%／平均共有 **1.00→1.85**／単一 bridge **60/60→42/60**。**中身の変化の方が大きい**——前者は首位が **The Deflated Sharpe Ratio（選抜バイアスと backtest overfitting）**に、後者は上位が **Quality Diversity / novelty search 系**に入れ替わった（before はそれぞれ「皮膚がん分類・AlphaGo」「脳波 ICLabel・化学 LLM」）。
* **正直に残す**: (a) **85% は再現していない**——本日の before 値は 35%/27% なので「85%→25%」と読んではいけない。(b) **上位10件占有率は 20%→30% / 0%→50% と悪化**したが、これは `diversify_head_by_bridge` の枠が「候補が引用する bridge の**すべて**が満杯のときだけ後回し」という判定なので、平均共有 bridge が 1 を超えると効かなくなるため＝**元からあった穴が新条件で表面化**したもの（**F-25** として起票・本日は未実装＝1日1件の規約）。(c) **「遠さ」は未改善**（F-01-R の残件そのまま）。(d) **2-hop 段の API 呼び出しは 2→11 回**＝F-11 の 429 が出やすくなる向き。
* 新規回帰5件（合成プールでの配分あり/なし A/B・走査順・OR 補充・1本失敗時の継続）。**420 → 425 tests: 425 pass。** 実 API は probe 2 テーマ（各 before/after）＋ `scripts/run_bybridge.py` end-to-end 2 回。
* **seihai 側への申し送り**: **S-26/S-67 の裁定（恒久除外）は測り直してから**。insights には「候補あたり平均 bridge」「通行 bridge 本数」を記録してほしい（上位10件占有率は F-25 のため単独では読めない）。seihai 側は一切触っていない。
