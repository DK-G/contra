...
## 2026-09-12 — 一致の「数」ではなく「網羅率」を測る。同じ関数が3つあるなら、欠陥も3つある（F-14/F-20）

**決定 (1)**: 関連度は **呼び手が宣言したキーワードの網羅率**（credit ÷ キーワード数）で測る。**絶対ヒット数に上限を掛ける形は、上限に達した瞬間に順位付けの情報を捨てる**——3本一致で上限に達していたため、4件のアンカーが全部 1.0 になり、順位は Reliability 単独に戻っていた（8週連続の「使える収穫ゼロ」の直接原因の一つ）。

**決定 (2)**: 語の照合は**正規化後の語境界**で行う。`cuped` が `cups-filters` に当たり、`quality-diversity` が "quality diversity" に当たらない、という**両方向の誤り**が同じ「素朴な部分一致」から出ていた。

**決定 (3)**: **同じ計算を3ソースが各自持っている状態を解消する。** GitHub / HuggingFace / Kaggle の `_theme_fit_score` は独立した3つのコピーで、欠陥も3つ同時に存在していた。共有モジュール（`src/pipeline/theme_fit.py`）に統合し、スケールと除外ペナルティだけを引数にした。**「同型の失敗が別のコードパスに残っている」は、この2日で3回目**（F-13-S: byserendipity の言語崩壊が bybridge に未移植／F-27: 自分で書いた「Field は主題適合ではない」が別経路に未適用／本件）。⇒ **失敗様式を直したら、同じ計算をしている場所を grep する**を定型手順にする。

**決定 (4)**: 数値の隣に**その数値の根拠**を印字する（`一致キーワード 4/5: …（名前/説明/topics）`）。定型文の散文は残っているが、**読み手が定型文を無視して判断できる材料**が同じ場所にある。F-13-I・F-17 と同じ「ラベルは導出元と同じ行に」の再適用。

**測ったが直さなかったこと**: 関連度係数の床 0.35 は、Reliability の可動域（30〜95）に対して係数の可動域（0.48〜1.0）が小さすぎるため、**関連度を正しく測っても順位が動かないテーマがある**。逆転には床 < 0.098 が必要（実測値からの計算）。床の当初目的（語彙一致ゼロのアンカーもランク可能に保つ）と衝突するので、**F-29 として分離し、複数テーマでの before/after を条件にした**。

---

## 2026-09-12 — 指標が改善して中身が悪化したら、処方ではなく指標を捨てる（F-25）

**決定**: `diversify_head_by_bridge` の枠は**旧挙動を既定のまま**にし、最頻 bridge 基準（`strict_cap=True`）は opt-in として残す。あわせて、**上位10件の bridge 占有率を「上位の質」の代理指標として読むのをやめる**。

**根拠（実測）**: 同一候補プールで枠の規則と大きさだけを変えたところ、占有率は単調に改善し（retrigger 40%→20%→10%→0%）、**同じ向きに中身が単調に悪化した**（主題直撃の data-snooping 検定・偽戦略検出が ESG・企業文化・可読性に置き換わる）。strategy_generation でも GP 他分野適用 10/10 の窓が半分になった。

**なぜこうなるか**: 上位窓の枠は **bridge の身元**で並べ替える操作である。プールが1本のハブに偏っていた時代（F-01 / F-23/24-R 以前）は、身元の偏り＝内容の偏りだったので枠が効いた。**プールの成り立ちを直した後は、最頻 bridge は「主題の正典」であることが多く**、身元で降格させることは主題直撃を降格させることと同義になった。**指標の意味が、上流の修正によって裏返っていた。**

**設計判断のポイント**:
1. **処方が書いてある起票を、実装したからといって採用しない。** F-25 は contra 自身が起票し、機序もコード読解で確定していた。**それでも既定にしなかったのは、中身の before/after を並べたから**であり、起票時に自分で「head-10 の中身を必ず並べて判断せよ」と書いた条件がそのまま効いた。
2. **外れた処方は消さず、opt-in として残す**（病的なプールが再び現れたときに再測できる）。テストも両方の契約を固定する。
3. **上流を直したら、下流の指標の意味を測り直す。** 「以前から測っている数字」は、前提が変わった瞬間に別の意味になる。S-26 の観測プロトコルはこの日をもって主指標を入れ替えるべき（F-26 の結論と一致）。

---

## 2026-09-12 — 主題で選んだ集合に、分野ラベルの絞り込みを重ねない（F-27）

**決定**: 取得レッグごとに「何で絞るか」を1つに決める。**埋め込み（意味）で選んだ semantic レッグに、Field（26分類のラベル）の絞り込みを重ねない**——home Field を先頭に並べる優先付けはしてよいが、**落とさない**。語彙レッグは逆に、同綴異義語対策として DB 層の Field 限定を掛け続ける（そこは絞りが効く段）。

**根拠（実測）**: home=`Economics` の3テーマで、Field 選別が落としていたのは rare-event simulation・GP の過剰適合と behavioural diversity・共分散ペナルティ型の backtest overfitting＝**どれもテーマの主題そのもの**。OpenAlex は手法寄りの仕事を CS / Decision Sciences / Mathematics に置くので、**「分野が違う」と「主題が違う」が一致しない**。strategy_generation では選別後の供給が 0 件、つまり**このレッグが一度も機能していなかった**。

**設計判断のポイント**:
1. **contra 自身が 9/04（F-13-I）に「Field は 26 分類の最粗レベルで主題適合ではない」と書いた。** その属性で、主題適合を測るために入れたレッグを刈っていた。**自分の書いた注記を、別の場所の実装に適用し損ねていた**——同じ知見が別のコードパスに移植されていない型（F-13-S の「byserendipity で既知の言語崩壊が bybridge に移植されていなかった」と同型で、2日連続で同じ型を踏んだ）。
2. **上流の選別を緩めると下流の境界も動く。** 交差候補の除外は `dominant_field_ids(seeds)`＝名簿の優勢 Field から作られるので、名簿に CS が入れば CS も除外側に回る。**「シードの選別を変える」は「何を交差と呼ぶか」を変える**ことであり、両方を同じ実測で確認しないと判断できない（本件は end-to-end の A/B で確認した）。
3. **緩める代わりに、緩めたことを計器に書く。** Subfield 一致率は下がるので、取得構成行に「名簿に残った semantic 由来 N 件・この数字は Field 一致率の分母に入れて読まない」を併記した。**数値の意味が変わったのに表示が同じなら、それは新しい失明である。**

**却下した代替**: (a) Subfield レベルで選別する——`resolve_subfield_ids` は厳格照合で多くのテーマで解決しないので、fail-open が常態化して選別が意味を失う。(b) off-Field を一定数だけ許す枠——公平配分マージが既に上限を作っており、二重の上限は挙動を読みにくくする。(c) 既定は旧挙動のまま opt-in にする——実測が3本とも同じ向きだったので、**既定を変えて opt-out を残す**方が失敗様式の再発を防ぐ。

---

## 2026-09-12 — 道具の自己記述は「書く」のではなく検証器から「生成する」。スコアが積なら、落選行に律速の因子を書く（F-21 / F-22）

**決定 (1)**: MCP スキーマの許容値・必須・長さは、**検証器の定数から生成する**。説明文に手で書いた制約は、書いた瞬間から乖離しうる（実際に5箇所で乖離していた）。生成に加えて、**「スキーマが提示する全値が実際に検証を通る」ことをテストで固定**する——これは説明文の文言をテストするのではなく、**提示と受理の集合が一致することをテストする**という形にしないと、次の乖離を捕まえられない。

**根拠**: F-21 は3回観測され、いずれも「呼び手が説明文どおりに書いて拒否された」。エラーは `is invalid` としか言わず、許容値を知るにはソースを読むしかなかった。**説明文・既定値・診断表示は、実装と食い違いうる**（seihai 側も S-89 / S-96 で同型を記録している）。⇒ 人間もエージェントも「読んで従う」側なので、**乖離しうる場所を減らす**のが処方であって、文言を直すのは1回限りの対症療法。

**決定 (2)**: **スコアが因子の積なら、閾値落選の行に必ず各因子と律速側を書く。** 積は「どちらが足りなかったか」を消す演算である。F-22 では `距離 × 構造` の積だけが表示されていたため、**「有用だが近い」と「遠いが浅い」が同じ数値の同じ落選行**になり、呼び手は毎週いちばん実務的な文献を落選として受け取っていた。

**設計判断のポイント**:
1. **スコアの定義は変えない。** 近い当たりが通らないのは較正不足ではなく設計上の帰結で、閾値では解けない。⇒ **直すべきは「読み取れないこと」**であって「通すこと」ではない。post-gate は決定論的検証であり採否の最終権限ではない（案X の分担）ので、**判断材料を増やして呼び手に返す**のが正しい層。
2. **ラベルの閾値は実バグで較正する。** `purpose_sim ≥ 0.70` は、seihai が「今週いちばん実務的」と書いた実際の候補（0.83 / 0.89 / 0.78＋CUPED 0.89）と、付けたくない側（0.66 / 0.64）の帯の間から採った。きりの良い 0.75 でも 0.80 でも実データの帯を切ってしまう。
3. **「この落選は正しい」と読ませない書き方にする。** 名指しの節に「採否の最終判断は呼び手にある／落選を理由に有用な文献を捨てないでください」を同じ場所に書いた。数値の隣に運用規約を置かないと、数値だけが独り歩きする（F-13-I の「緑のスカラ」と同型）。

**却下した代替**: (a) 閾値を下げる／`percentile_gate` を外す——F-15 の相対性問題は動くが F-22 は解けない（積の定義の問題）。(b) 近い候補の `mechanism_dist` を底上げする——計器を偽装する行為で、F-13-I で否定した「粗い属性を正常値で表す」と同型。(c) 落選候補を出力に自動昇格——呼び手の判断を contra が奪う。

---

## 2026-09-11 — 埋め込み検索に渡す文は、呼び手の言語ではなく検索対象の言語で書く。「0 件」は段ごとの内訳を付けて返す（F-13-S）

**決定**: bybridge の semantic シードレッグは、テーマ本文が非英語なら**英語の `keywords_include` に差し替え**、呼び手が英語の擬似アブストラクト（`seed_semantic_text`）を渡せばそれを優先する。問い合わせは 1,200 字で文末切り詰め。そしてこのレッグの供給は**「検索文の出所 → 返却 → 言語構成 → abstract 無しで −N → home Field 外で −N → 供給」**の内訳付きで診断に出す。

**根拠（実測・実 API）**: 「semantic レッグ供給 0 件」は5回観測されていたが、seihai のテーマ本文は日本語で、埋め込み検索は**意味より先に言語で近傍を取る**（返却 50 件中 ja 24/46/45）。日本語レコードは選別段で全滅し、旧経路は 4/4 回 0 件を再現した。英語 keywords に差し替えると返却は全件英語になり、供給は 0/0/0 → 6/0/14。byserendipity 側では同じ現象を F-16 の「facet を日本語で書くと日本語文献で埋まる」として既に知っていた——**同じ endpoint を使う別レッグに、その知見が移植されていなかった**。

**設計判断のポイント**:
1. **呼び手の言語と、検索対象コーパスの言語は別物**。bybridge の 2-hop が必要とする引用グラフは英語圏に偏っている（`seed_language:'en'` の既定もその判断）。**同じ理由で、検索文も英語でなければ近傍がそもそも英語圏に落ちない。**
2. **自動翻訳はしない**。LLM を呼べば翻訳できるが、bybridge の key-free 経路を壊す。英語 keywords は既に呼び手が書いている英語の資源で、**擬似アブストラクトの作文は呼び手（エージェント）の仕事**——委譲経路（案X）の分担と同じ。
3. **「0 件」を1つの数で返さない**。取得失敗・言語不一致・Field 選別は同じ 0 に潰れ、呼び手はそれを「テーマが飽和した」と誤読しうる（9/04 の F-16 で実際に起きかけた）。**F-13-I の「計器は測っていないものを名乗る」の延長で、ゼロには必ず落ちた段を付ける。** 実際この内訳を入れた直後に、2つ目の律速（home Field 選別が主題直撃の CS 分類論文を捨てる＝F-27）が見えた。

**却下した代替**: (a) 504 に対するリトライ回数の増加——504 は短い英語問い合わせでも一過性に出ており、主因ではなかった。F-11 の据え置き判断も尊重。(b) 日本語本文のまま `seed_language` ゲートを外す——日本語の近傍は主題でなく言語で集まっているので、ゲートを外しても主題は戻らない。(c) semantic レッグの home Field 選別を同時に外す——下流の Field 一致率計器と L0 外判定に波及する設計判断なので F-27 として分離（1日1件）。

---

## 2026-09-08 — 集合から抽出するときは、集合の要素の大きさが取り分になる。公平配分は「並べ替え」ではなく「取得」の段でしか作れない（F-23/F-24）

**決定**: 複数の情報源を OR で1本のクエリにまとめて上から取る形は、**各源の大きさで取り分が決まる**。多様性を保証したいなら**源ごとに問い合わせて取り分を切る**（配分）。後段の並べ替えや表示枠は、**すでに偏って集められたものの順番しか変えられない**。

**根拠（実測）**: bybridge の 2-hop スキャンは `cites:B1|...|B50` を投げていた。7週分の「巨大ハブ吸着」の残り——F-01-R（phantom bridge の除去）の後もなお 62%（`R` の引用・9/07）／85%（`Theory of the firm`・9/08）が1本の bridge を通っていた現象——は、この一点で説明がつく。**種別（道具か文献か）でも被引用数の閾値でもない**: 9/07 の最頻 bridge はソフトウェア、9/08 は経済学の正典で、共通するのは「集合の中で一番大きかった」ことだけ。**2026-08-18 に中心性ペナルティが実測で棄却された理由もここにある**——大きさは*症状の相関物*であって原因ではないので、大きさで罰すると本物の祖先文献を巻き添えにする。**原因は大きさが取り分に直結する取得の形の方**だった。

**設計判断のポイント**:
1. **同型の処方が既に2つある**: F-07 のシード配分（1シードがプールを埋め尽くさない）、F-18 の facet 公平配分（最後の facet が飢えない）。**bridge 段だけが配分を持っていなかった**。⇒ **「上流の段に配分があるか」を、故障を見たときの定型の問いにする。**
2. **表示層の枠を、プール層の対処と数えない**。2026-08-22 の `diversify_head_by_bridge` は「上位10件の見た目」を直し、**同じ名前の問題が直ったように見せた**。実際には収集済みの偏りを並べ替えただけで、seihai は5週間その差を観測し続けた。**「どの段で直したか」を診断に書く**（本件で配分の存在と上限を印字したのはこのため）。
3. **配分は上限であって床ではない**——不足分は従来の一括走査で補充する。多様性のために recall を下げるのは、**収穫ゼロを多様性と呼ぶ**ことになる（F-18-R で一度通った道）。
4. **順序が結果を決める設計に変えたら、順序を捨てている場所を探す**。プール構築は多様性順に並べていたのに、収集段へは `sorted()`＝id のアルファベット順で渡っていた。OR 1本の時代は無害だったので誰も気づかなかった。**「今まで無害だった雑さ」は、設計変更の瞬間に有害になる。**

**却下した代替**: (a) bridge の**種別**で切る（F-23 が提案した「ソフトウェア・データセットを除外」）——9/08 の最頻 bridge が正典文献だったので**この様式を説明できない**うえ、道具の共引用が有効な主題を殺す。(b) **被引用数の上限**——2026-08-18 に実測で棄却済み、F-01-R でも「大きさは判別に使えない」と再確認済み。(c) 上位窓の枠を厳しくする——(2) のとおり段が違う。

**代償として引き受けたもの**: 2-hop 段の API 呼び出しが 2 → 11 回（F-11 の 429 リスクが上がる方向）。1本あたり1ページしか読まないので上限はプール本数で、**取得できなかった bridge は静かに飛ばす**（他の 49 本を巻き添えにしない fail-open）。

---

## 2026-09-04 — 計器は「測っていないもの」を必ず名乗る。粗い属性の緑のスカラは、故障を隠す装置である（F-13-I）

**決定**: 診断に数値を出すとき、**その数値が測っていない範囲を同じ行に書く**。そして**単一のスカラを「正常」の合図として出さない**——複数の粒度（Field / Subfield / Topic）と、その数値を支えた取得経路の供給数を必ず併記する。

**根拠（実測）**: `シード主題整合 (F-13): home分野一致 20/20 = 100%` は、NAFTA・ブロードバンド・COVID 政策トラッカーで構成された名簿に対して出た表示である（seihai 9/04）。表示は**嘘ではない**——名簿は確かに全件 `Economics, Econometrics and Finance` だった。嘘だったのは**名前**（「主題整合」）と、**100% という緑が「調べた結果、問題なし」に見えること**である。同じ名簿を Subfield まで降ろすと **1/20 = 5%** で、上位トピックは `Economic Growth and Productivity / Healthcare Policy and Management / COVID-19 Pandemic Impacts`＝一目で外れが読める。

**設計判断のポイント**:
1. **計器を入れたら、計器自身の縮退様式を数え上げる**。この計器は3つ持っていた: (a) 外れているのに 100%（9/04）、(b) `scope_field` 未解決で判定不能（9/03）、(c) 語彙衝突そのもの。**(a) が最悪**で、(b) は少なくとも「測れていない」と自白している。⇒ **迷ったら (b) 側に倒す**——`resolve_subfield_ids` を厳格照合にして、緩く当てるくらいなら判定不能を返す設計にしたのはこの理由。
2. **数値ではなく名前を読ませる**。ドリフトは率では捕まらないことがある（Finance の中の無関係な主題）。上位トピックの**名前**の印字は閾値を持たず、しかし記録された全事例で人間がそれを読んで気づいた。**閾値のある指標と、閾値のない一覧を、両方出す。**
3. **閾値は実バグと健全例の両方に当てて決める**（F-17 で決めた規約の再適用）。5% / 65% / 71% の3実測から 0.35。きりの良い 0.5 は健全帯に食い込む。
4. **外れた仮説を消さずに残す**。トピック分散（範囲が重なる）と `/text/topics`（衝突語に対してドリフトそのものを承認する）は、実測付きで `field_observations_seihai.md` に残した。とくに後者は「一見もっともらしい処方が、捕まえたい故障を正常と判定する」型で、次に同じ道を通る担当が最も落ちやすい。

**計器を外部依存から切り離した**: Subfield 名の照合語彙は **run が既に取得した metadata** から集める。固定表（`/subfields` の 252 件）を凍結する案から切り替えた理由は2つ——(1) 本日 OpenAlex が 40 分以上 429 を返し続け（F-11 の再現）、**計器の実装が API の機嫌に依存すると、直したい日に直せない**ことが実演された。(2) 固定表は陳腐化する。代償は「宣言した主題が名簿に1件も無いと 0% でなく判定不能になる」＝**最も重いドリフトが最も弱く出る**向きで、これは `data/openalex_subfields.json`（任意のキャッシュ）を置けば塞がる。**弱く出る側の誤りを選んだ**のは、この計器の失敗が「緑と偽る」ことだったから。

**却下した代替**: (a) `100%` の表示だけをやめて数値を隠す——呼び手は率で読む習慣があるので、隠すと別の粗い代理を自作する。(b) Subfield 名への緩い照合（`medicine` → `Internal Medicine` ほか多数）——粒度を下げた意味が消え、同じ失明を細かい単位で作り直す。(c) テーマ本文の埋め込み距離——鍵が要る（F-11 の未対処部分と衝突）。**鍵無しで測れる代替＝semantic レッグ供給数の報告**を採り、0 件なら「未検証」と明言する形にした。

---

## 2026-09-01 — ラベルは導出元の数値と同じ行に置き、別の量に同じ名前を名乗らせない（F-17）

**決定**: 定性ラベル（高/中/低）は**必ずそれを導いた数値と同じ行に描画する**。また、ある量を「〜度」と名付けて表示するとき、**その名が指す量以外から導出してはならない**——`関係度` は theme 関連度の帯であり、品質（Reliability）の帯ではない。

**根拠（実測）**: `structured:true` の byrepo は6日間、Reliability の帯を `関係度` の名で描画していた。seihai 8/27: 唯一有用な frouros（関連度 1.0・品質 62）が「中」、無関係な2件（関連度 0.33・品質 ≈90/83）が「高」。**順位は F-03 で正しかった**のに、読み手はラベルを先に読むので「正しく読んだ結果として誤る」。同じ出力の1行下に `theme関連度 0.33` が印字されていたが、行が分かれていたので矛盾に気づけなかった。

**設計判断のポイント**:
1. **順位に効かせる量を変えたら、同じ量を名乗るラベルの生成経路を数え上げる**。F-03 は順位だけ直した。姉妹経路の網羅チェック（seihai 台帳の再発様式）。
2. **帯の閾値は実バグで較正する**。0.33（3語中1語ヒット）が「低」に落ちること、下限が既存の低関連度警告（0.35）と一致することの2点で決めた。きりの良い 0.3/0.7 ではない。
3. **情報を消さない**。旧ラベルが語っていた品質の帯は `Reliability Score` 行に別名で残る。関数 `_reliability_level` も残置し「関係度ではない」と明記。
4. **ラベルが正しくなっても係数の欠陥（語の一致＝F-13/F-14）は残る**。全件 1.0 の run では全件「高」になるが、それはラベルが正しく欠陥を映している状態であり、隠すより良い。

**却下した代替**: (a) ラベルを消して数値だけにする——読み手は帯で読むので、帯を消すと数値の読み替えを全員に強いる。(b) 品質×関連度の合成帯——順位スコアの帯と同じものになり、「関係度」という名に戻って嘘になる。

---

## 2026-09-01 — 呼び手のミスを検出したメッセージは、呼び手が次に取るべき行動を名指しする（F-19）

**決定**: 検証失敗を報告するとき、**原因が分かっている失敗を、原因の分からない失敗と同じ文言で報告しない**。接地契約では `照合不能`（干し草の山＝呼び手が送った材料が空）を `不一致`（干し草はあるが針が無い）から分離し、**原因と復帰手順を同じ文に書く**。原因が別ブロックにしか書かれていない状態は「書いてある」に数えない。

**根拠（実測）**: seihai は逐語で正しい `source_quote` を送って 5/5 接地失敗を受け取り、**最初に自分の散文を疑い直した**。contra は同じ出力の数行上で `⚠ 材料欄が欠けたまま送信されています（title, abstract, …）` と正しく警告していたが、それは「該当欄は空のまま描画されます」＝**体裁の問題として**書かれており、接地失敗の原因であることはどこにも書かれていなかった。**2つの正しいメッセージが、因果を欠いたまま並ぶと誤った行動を誘発する。**

**設計判断のポイント**:
1. **空の干し草に対する不一致は不一致ではない**。`quote not in haystack` は `haystack == ""` のとき常に真になる——これは照合の結果ではなく**照合が成立しなかった**という別の事象で、別の名前を与える。
2. **診断の誘導先は、呼び手が持つ仮説のうち最も高くつくものを避ける**。「存在しない」は事実として正しいが、呼び手の仮説空間では「自分の引用が捏造だった」が最初に立つ。これは**最も検証コストが高く、最も外れやすい**仮説である。
3. **甘くしてよいのは原因が特定できるときだけ**。全欄が揃った不一致には注記を一切付けない（回帰テストで固定）。これを緩めると接地契約そのもの（A1 = F-04 の捏造を構造的に不可能にする）が空洞化する。
4. **部分的な欠落には「不一致だが原因候補はこれ」**。title だけ echo された場合、干し草は空でないので判定は不一致が正しい。ただし abstract 由来の引用は原理的に一致しえないので、その事実を併記する。

**却下した代替**: (a) **contra 側が保持する原本と照合する**——委譲アーキテクチャ（案X）の前提を壊す。照合対象が呼び手の提出物であることは仕様であって不具合ではない（呼び手が読まずに引用を捏造する経路を塞ぐのが A1 の目的）。(b) **材料欄が欠けていたら finalize をエラーで拒否する**——スコアは有効なので通す価値があり、既存の呼び手を壊す（`ECHO_RECOMMENDED` は hard-required ではないという F-09 時点の判断を維持）。

**この判断の姉妹形**: F-18-R / S-68「情報が無いことを中立値で表すな」。あちらは**沈黙を成功に見せる**欠陥、こちらは**呼び手のミスを呼び手の別のミスに見せる**欠陥で、どちらも「計器が実際に起きたことと違うことを言う」型。

---

## 2026-08-28 — シード検索は「語彙＋分野境界＋テーマ本文」の三層で引く（F-13）

**決定**: bybridge のシード取得を三層構成にする——(B) 全語彙クエリ（フォールバック含む）に home Field 限定、(C) テーマ本文の semantic 検索レッグを公平配分で併走、(A) 名簿の分野整合を常時計測。各層は独立フラグで可逆、すべて fail-open。

**根拠（実測と文献の合流）**: 同一テーマで語彙シードは7回以上ドリフトし、semantic 検索（80語の擬似アブストラクト）は主題に当たり続けた＝故障は OpenAlex でなく語彙経路に局在。コード読解で「テーマ本文がシード検索に一度も入らない」「field_ids 未配線」「フォールバックが無限定」を確認。bynote 調査（Deep Research 10ソース）の標準処方——短クエリの多義衝突には DB 層分野フィルタ＋長文擬似文書クエリ＋取得後分布検知——と正確に一致した。実測: home一致 50%→100%、semantic シードは referenced_works を持ち bridge シードとして成立。

**設計判断のポイント**:
1. **フォールバックにも境界を残す**。「精密クエリが外れたら緩く引き直す」は recall の床として正しいが、**分野境界まで捨てると多義衝突の扉になる**（8/27 の耳鳴り・農牧シードはこの扉から入った）。recall の床と分野の壁は独立に管理する。
2. **「近傍」は語彙でなく本文で定義する**。シードの資格は「テーマと同じことを論じているか」であり、キーワード数語はその貧しい代理だった。テーマ本文そのものを問い合わせにする semantic レッグが、代理でなく本体を検索する。
3. **警告閾値は実バグと実健全の両方で較正**（50%＝旧経路の実測値。限定後の健全 run は ~100% に張り付くので、50%割れ＝限定の失敗か無効化の信号）。分野未解決は 0% でなく「判定不能」（S-68 同型）。

**却下した代替**: 中心性/人気度ペナルティ（F-01 で棄却済みの軸）、キーワード調整の呼び手依存（8/26-28 の対照で「語を減らす/増やす」は両方反証済み）、シード事後ゲート単独（DB 層で防げるものを取得後に捨てるのは枠の無駄＋空プール化リスク）。

---

## 2026-08-28 — 複数の入口から集めるときは、上限を「早い者勝ち」で配らない

**決定**: 複数の独立した取得源（facet / クエリ / シード群）から候補を集める段では、**取得順に連結して上限で打ち切る**のをやめ、**全源を必ず叩いてから上限を源の間で配分する**（`facet_fair_share`・既定 on）。

**根拠（実測）**: byserendipity の raw_only 経路は facet を順に引いて `len(works) >= max_count` で break していた。semantic は1リクエスト最大50件・上限60件なので、**3枚目の facet はリクエストすら発行されない**。A2 距離プロトコルが facet を Near → Far → Very Far の順に並べる以上、**打ち切られるのは常に最遠の1枚**＝プロトコルが買おうとしていた当のもの。実測 A/B で facet 別提出は 42/18/0 → 20/20/20、候補総数は 60 のまま（薄い源は枠を譲るので**公平配分は収量を下げない**）。

**一般化**: **「多様性のために複数の入口を用意する」設計と「上限に達したら止める」実装は、黙って衝突する。** 衝突は必ず**最後の入口＝設計上いちばん遠い入口**を食う。多様性を目的に列挙した源は、順番が意味を持つ（遠い順に並べる）ので、順序依存の打ち切りが最も高価なものを最初に捨てる。**列挙した源には必ず枠を割り当てる。**

**計器が先、処方が後（3回目）**: seihai は原因について2つの仮説（OpenAlex の足切り／ホームドメイン除外の巻き込み）を立て、**どちらも外れていた**。真因を指していたのは仮説ではなく**計器の数字**——「facet は3枚なのにリクエストは2件」。F-01（2026-08-25）・F-07（2026-08-18）に続き、**診断ブロックの不整合な数字が仮説より先に真因へ着いた3例目**。⇒ **不整合な計器値は「些細な表示の揺れ」ではなく、最優先で追う手掛かりとして扱う。**

**「影響なし」と書かない**: `run_stats_caveat` は HTTP 応答しか見ていないのに「結果への影響なし」と断定していた（欠損した facet がまさにそこにあったのに）。**測っていない範囲について中立・安全を申告する文言は、故障を不可視にする**（seihai 側 S-68 と同型）。⇒ 診断文言は**自分が何を見ていないか**を書く。

---

## 2026-08-25 — bridge プールの選別軸は「中心性」ではなく「実在性」

**決定**: bybridge の bridge プールに対して、**中心性ペナルティ／degree 上限は採らない**。代わりに **生存確認（OpenAlex が id を解決するか）** をゲートにする。

**根拠（実測）**: F-01 として7週間記録されてきた「巨大ハブ吸着」の最頻 bridge `W4285719527` は、**そもそも作品として存在しない**（404 / `ids.openalex` で count 0）にもかかわらず **490万件の参考文献リストに id が残っている**。同一プール内の死んだ id 5本の被引用数は 4,906,577 / 4,194 / 2,749 / 554 / 59 とばらつき、逆に生きた最大 bridge は Fama-French(27,948)・GARCH(22,513) という主題ドメインの正統な基礎文献だった。⇒ **どこに閾値を置いても、小さい phantom を見逃し本物の祖先文献を罰する。** 大きさは判別に使えない。

**この判断の系譜**: 2026-08-18、F-02 の計器（シード一覧・bridge 集中度）が最初に中心性ペナルティ仮説を否定し、真因を F-07（重複 proceedings によるプール占領）へ差し替えた。本日はその同じ計器が出していた**「最頻 bridge の名前が解決できず生の id のまま出る」**という兆候を追って、さらに下の層に到達した。**「計器が先、処方が後」が2回続けて正解だった実例**として残す。

**設計上の一般化**: seihai 側の処方「**採用の前に生存確認**」（0発火の戦略設計を採らない → contra では F-12 のシード側ゲート）は、**パイプラインの各ホップで独立に必要**である。シードに適用しても、bridge に適用していなければ次のホップで同じ形の沈黙が起きる。**「出力を持ち得ない要素が枠を占める」は、段ごとに潰す。**

**fail-open の二重化**: 通信失敗だけでなく「全件 dead」という応答も**真実ではなく不良応答として扱う**。ゲートがプールを空にすると、呼び手から見て「収穫ゼロ」と区別がつかない（F-12 で実際に起きた形）。**安全側は常に「落とさない」。**
