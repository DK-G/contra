        ...
        if not works:
            return {
                "content": [{"type": "text", "text": (
                    ...
                    f"または履歴除外 {len(used_ids)} 件で全滅）。\n"
                    # F-16 (2026-09-04): this message used to名指しする原因が「facet が近すぎる」
                    # 「テーマが飽和」の2つだけで、実際の原因（クエリ長超過による 400）に一言も
                    # 触れていなかったため、呼び手は生きているテーマを「飽和した」と結論しかけた。
                    # 上流の一過性障害を候補の性質として読ませない（S-112 と同じ規約）。
                    "**まず下の facet 内訳の `status` を読んでください。**\n"
                    "・`取得失敗 (… 400 …)` ＝ クエリ長超過が最有力です（contra は 1,200 字で切り"
                    "詰め、400 を受けたら1回だけ短縮して再取得しますが、それでも落ちることがあります）。"
                    "pseudo_abstract を約80語・structure を5文程度に縮めて投げ直してください。\n"
                    "・`取得失敗 (… 429 …)` ＝ この IP の共有クォータ枯渇です（by\\* は同一クォータを"
                    "共有・F-28）。時間をおいて再実行してください。\n"
                    "・`取得失敗 (… 504 …)` ＝ 上流の一過性障害です。**同じ facet を単独で投げ直す**と"
                    "通ることが多く、実測で復旧しています（S-112）。\n"
                    "・`棄却 (home_converged)` ＝ ここで初めて「facet が近すぎる／テーマが飽和」の"
                    "疑いが立ちます。facet をより遠い具体ドメインへ見直してください。\n\n" + facet_diag
                )}],
                "isError": False,
            }
