...
- Track A の「関係軸ラベル」の語彙セット（自由生成か固定リストか）
- Track B の接続点タイプ8種の優先順位・論文がない場合の補充ルール
- Track A の関係度は5段階ラベル（高/中高/中/中低/低）か数値（1-10）か未確定
- GeminiCLIへの入力フォーマットをTrack A/B構造に合わせた更新（現在は旧4章構造）
- `classify_stub` の本実装をLLMベースにするか、ルールベースにするか未確定
